...
**`bun run lint` is red on the current tree**, both halves: `prettier --check .` on 98 files (the figure drifts as files are touched) and `eslint .` on **5** errors. Because the script is `prettier && eslint`, a prettier failure means eslint never runs — so anything that needs eslint must invoke it directly. This is why CI reports lint without blocking on it. Do not "fix" it by reformatting the tree as a side effect of unrelated work.

**The eslint count is ratcheted, and the number it ratchets is not the one you get by running eslint yourself unless your ignores are current.** `bun run lint:ratchet` asserts 5 and fails in both directions — up, because new errors are not accepted; down, because a baseline nobody tightens only rots. It runs as a blocking step inside `test & build`, not in the non-blocking `review` job, since a ratchet that cannot fail the merge is a report.

The 5 replaces the 11 recorded at `d2b1f5f`, but the gap that mattered was neither figure: **a machine that had run `supabase start` counted 213.** `eslint.config.js` derives its ignores from `includeIgnoreFile(.gitignore)`, which reads only the file it is handed — so `supabase/.temp/`, ignored by the Supabase CLI's own nested `supabase/.gitignore`, was invisible to it, and eslint walked into the running stack's edge-runtime scratch code for 208 of those errors. That tree also holds `start-secrets/`, which is not somewhere a linter should be reading. It now carries an explicit `{ ignores: ['supabase/.temp/**'] }`; without it the ratchet would compare a number that depends on whether your local stack is up.
