...
than guessed. Compiled 2026-08-15 against the page corpus at that date; **scope, counts and the
`U1`/`U6` rows re-measured 2026-08-30 against the corpus at `16b04ce` (39 pages, 10 types), which
added `U25` and `U26`.** Where a 2026-08-15 figure was superseded the old value is kept in the text
rather than deleted, so a reader who remembers it can see it was replaced by measurement.
...
## Scope — the ten types currently in use

**Re-measured 2026-08-30 against the corpus at `16b04ce`: 39 pages, 177 sections, 66 steps, 10
distinct `type` values.** The 2026-08-15 figure of 29 pages / 8 types is superseded — `#97` added
ten proposed pages across all ten Karl types, which put `Step by step` and `Location` into use for
the first time and moved both out of "Types not yet in use" below.

Types Karl offers but this mockup still does not use (`Data story`, `Document Collection Search`,
`Event`, `Form`, `Meeting`, `News`, `Profile`) are out of scope by definition and are not mapped
here.

**Read the evidence column before trusting a row.** The eight original types were expanded in the
live form and carry block internals. `Step by step` arrived with its `steps` block already expanded
at E1, because the 2026-08-15 pass captured it while resolving `U3`. **`Location` did not** — it was
captured at panel-name depth only, on the explicit reasoning that nothing depended on it. Something
does now (`ehOffice`), so its row is marked E2 and carries `U26` rather than being presented as
mapped.

| `type` value          | Pages  | Sections | Steps  | Page keys                                                                                                                                                                                                                                                                                        | Nested-block evidence             |
| --------------------- | ------ | -------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------- |
| `Transaction`         | 16     | 51       | 42     | `inspectorLookup`, `inspectionPrepInitial`, `inspectionPrepFollowup`, `findRecords`, `findHotelRecords`, `findViolations`, `payFee`, `publicRecords`, `insectsReport`, `filthReport`, `rodentsReport`, `sroHotelReport`, `noticeOfViolation`, `tenantNoticeSteps`, `appealNotice`, `payCitation` | **E1** — full                     |
| `Information`         | 7      | 43       | 5      | `article11Compliance`, `scopeInfo`, `ownerGuidance`, `mosquitoControl`, `tenantRights`, `afterReport`, `whoToCall`                                                                                                                                                                               | **E1** — full                     |
| `Resource Collection` | 4      | 19       | 0      | `verminResources`, `recordsHub`, `ownerHub`, `languageGuides`                                                                                                                                                                                                                                    | **E1** — full                     |
| `Campaign`            | 3      | 20       | 0      | `ipmEducation`, `mosquitoWorkshop`, `ratFreeBlocks`                                                                                                                                                                                                                                              | **E1** — full                     |
| `Step by step`        | 3      | 9        | 19     | `stepPestReport`, `stepCorrectViolation`, `stepPrepUnit`                                                                                                                                                                                                                                         | **E1** — `steps` expanded         |
| `Report`              | 2      | 15       | 0      | `article11Guide`, `inspectionsAnnual`                                                                                                                                                                                                                                                            | **E1** — full                     |
| `Topic`               | 1      | 5        | 0      | `healthyHousingTopic`                                                                                                                                                                                                                                                                            | **E1** — full                     |
| `Agency`              | 1      | 6        | 0      | `pestsTopic`                                                                                                                                                                                                                                                                                     | **E1** — full                     |
| `About us`            | 1      | 4        | 0      | `aboutHhvcTeam`                                                                                                                                                                                                                                                                                  | **E1** — full                     |
| `Location`            | 1      | 5        | 0      | `ehOffice`                                                                                                                                                                                                                                                                                       | **E2** — panel names only (`U26`) |
| **Total**             | **39** | **177**  | **66** |                                                                                                                                                                                                                                                                                                  |                                   |
...
`slug` sits on the Promote tab, so a page cannot be saved from the Content tab alone.

**Recomputed 2026-08-30 against the ten in-use types.** The old sentence read "seven of the eight
types cannot be created from mockup data without one value supplied by hand" and is superseded, in
both directions:

- **Nine of the ten in-use types require `primary_agency`.** Only `Agency` does not, because it _is_
  the agency. (`Step by step`'s requirement is measured; `Location`'s is inferred from the pattern,
  not read off its form — see `U26`.)
- **The mockup now has a field, on one type.** `primaryAgency` is present on **16 of 16**
  `Transaction` pages and **0 of the other 23** pages across nine types. So `U6` is narrowed, not
  closed — and narrowed unevenly.
- **A label is not a page reference.** Every one of those 16 stores the string
  `'Healthy Housing and Vector Control'`, while `primary_agency` is a page chooser restricted to
  `Agency` pages. The approved target exists — `sf.gov/departments--healthy-housing-and-vector-control`,
  the page keyed `pestsTopic` — so the value is resolvable, but the mockup field does not hold a
  reference to it.

The practical count is therefore unchanged at build time: **nine of the ten in-use types still need
`primary_agency` supplied by hand**, because a label does not satisfy a page chooser.
...
**16 pages, 51 sections, 42 steps** (re-measured 2026-08-30 at `16b04ce`; was 14 / 47 / 34 on
2026-08-15, before `#97` added `appealNotice` and `payCitation`). The heaviest-used type in the
corpus.
...
| Primary agency                                  | `primary_agency`         | **yes**                      | single                                              | page chooser → `Agency` only                                                                              | `primaryAgency` — 16/16, but a **label**, not an Agency page reference (`U6`, narrowed)        |
...
| Partner agencies                                | `partner_agencies`       | not recorded                 | repeatable                                          | page chooser → `Agency` only                                                                              | `partnerAgencies[]` (11 of 16 pages)                                                           |
...
Karl offers seventeen content types and this mockup now declares **ten**. The other **seven** were
captured in the same read-only session on 2026-08-15, at panel-tree depth: raw field names and panel
order for all of them, chooser contents and block internals only where a plausible HHVC page would
need them. **Nothing in this section maps to a page that exists** — it answers "which type should
this page be?" before the per-type sections above answer "what fields does it have?".

> **Two types left this section on 2026-08-30.** `#97` created pages for `Step by step`
> (`stepPestReport`, `stepCorrectViolation`, `stepPrepUnit`) and `Location` (`ehOffice`), so both are
> now in scope and appear in the Scope table above. Their subsections are retained below rather than
> deleted, because the capture they record is the only capture either type has — but read them as
> in-use documentation now, and note the asymmetry: **`Step by step` was expanded to its `steps`
> block internals and `Location` was not** (`U26`). `Step by step`'s subsection also carries the
> resolution of `U3`, which is why it was expanded ahead of need.
...
| -------------------------- | -------------------------- | ----------------------------------------------------- | ----------------------------------- |
| ~~**Step by step**~~       | `stepbystep`               | **NOW IN USE** — 3 pages via `#97`                    | panels + `steps` block internals    |
| ~~**Location**~~           | `locationpage`             | **NOW IN USE** — `ehOffice` via `#97`                 | panels only — see `U26`             |
...
cost or things-to-know — and `U3`'s `afterReport` is exactly that question. `#97` then built three
`Step by step` pages directly, so the type is no longer hypothetical.

> **The "resolved by rewriting the notes" claim above has regressed — see `U25`.** It was true on
> 2026-08-15. As of 2026-08-30 five `karl` notes on `Transaction` pages again name `Steps`,
> `Transaction link` or `information_section`, and two of the five arrived on pages `#97` added. The
> sweep worked; what failed is that nothing prevents a new page from copying the old template.
...
table above records as unmappable, true only for the ten types in use.
in use.
...
| ------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `audience[]`                                      | all 39 pages                                                    | **Corrected 2026-08-30 — it DOES migrate.** It was recorded here as "not a Karl concept"; a published page settles otherwise. `sf.gov/manage-covid-19-schools-childcare-and-youth-programs` renders **"Who this information is for"** as an H3 inside its grey "What to know" box, audiences as a bulleted list — i.e. one `things_to_know` (`title_and_text`) entry. E4 governs this: no guidance document can say what a template outputs. What is unresolved is the BUDGET, not the destination — Things to know is capped at 2 entries and 14 of the 15 pages carrying it already spend both, so placing an audience means choosing which entry it replaces. The transcript now reports this per page instead of suppressing it. |
| `reading`                                         | all 39 pages                                                    | **Not migrated.** HHVC grade-level target                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `editorNote`                                      | 36 pages                                                        | **Not migrated.** QA guidance — equivalent to a Wagtail workflow comment, not page content                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `editorStatus`                                    | 22 pages                                                        | **Not migrated.** Review-state, mockup only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `kind`                                            | every section                                                   | **Not migrated.** Drives tag colour in the mockup                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `karl`                                            | every section (schema-required), all 66 steps, 113 of 215 cards | **Not migrated** — it is the instruction _about_ the migration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `topicTag`                                        | 3 pages                                                         | **Not migrated.** Unmapped metadata (mockup values store Agency labels, not Topic chooser references)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `seoTitle` / `metaDescription`                    | all 39 pages                                                    | Verified mapping to Wagtail Promote tab fields (`seo_title` / `search_description`, closed `U11`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `callout.title`                                   | several                                                         | **No home.** Every Karl `Callout` is a bare rich text field (`U2`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `primaryCta`                                      | **0 pages**                                                     | Schema-declared, unused. No home on the ten types in use — but `call_to_action` exists on **Agency** (in use, unused by `pestsTopic`) and on **Event** (still not in use). So it is unmappable today rather than unmappable in principle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `printVersionUrl`                                 | **0 pages**                                                     | Schema-declared, unused. Report's "Print version" field exists and is a document chooser, not a URL                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| section-level `button`/`buttonUrl` outside a step | **8 Transaction** (+ 5 in `spotlight`, which are fine)          | **Re-measured 2026-08-30 at `16b04ce`: 13 in total — 8 on Transaction with no home (`U1`), plus 4 Campaign and 1 Topic inside `spotlight` sections, which are fine.** Supersedes both prior figures, which disagreed with each other: the `U1` register row said "8 Transaction, 4 Report", this row said "5 Transaction, 2 Campaign, 1 Topic". Report carries none.                                                                                                                                                                                                                                                                                                                                                                 |
...
| ----- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `U1`  | open     | **Section-level buttons outside a step. Re-measured 2026-08-30 at `16b04ce`: 8, all Transaction** — `inspectorLookup`, `findRecords`, `findHotelRecords`, `findViolations`, `publicRecords`, and the "How your report is processed" section on each of `insectsReport`, `filthReport` and `rodentsReport`. Transaction's only `Button link` slot is inside a `what_to_do` Section, so none has a home; the three report-page ones are worse, because they sit in `things_to_know`, which offers no `Button link` block at all. **Report carries none.** This row previously said "8 Transaction and 4 Report" while the "Mockup fields with no Karl destination" table said "5 Transaction, 2 Campaign, 1 Topic" — two in-document figures disagreeing means at least one was never derived from the corpus, so the count above names its pages and can be re-derived. The 4 Campaign and 1 Topic buttons sit in `spotlight` sections and are fine. The Help Center's "no more than one button per page" editorial rule is unaffected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Digital Services decision                                  |
...
| `U6`  | narrowed | **Narrowed 2026-08-30, and unevenly — do not read it as closed.** `primary_agency` is required on **9 of the 10** in-use types (all but `Agency`). The mockup now has a `primaryAgency` field, but only on one type: **16 of 16 `Transaction` pages carry it; 0 of the other 23 pages across nine types do.** All 16 store the label `'Healthy Housing and Vector Control'`, while `primary_agency` is a page chooser restricted to `Agency` pages — so nothing is yet saveable from mockup data without a hand-supplied value, which is what this row originally said. The approved destination exists: `pestsTopic`, `sf.gov/departments--healthy-housing-and-vector-control`. **Two reviewer-facing places still say the field does not exist** — `src/lib/legacy-core/karl-blocks.js:417` and `:1946`. Also still open: `Part of` (Information) and `good_for_community` (Transaction).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Wire `primaryAgency` to a page reference, not a label      |
...
| `U24` | closed   | **Closed 2026-08-29.** `article11Guide`'s page-level Spotlight button was shortened from "View Health Code Article 11" (**27 characters**) to "View Article 11" (**15 characters**) in `src/lib/data/health-code-article-11.ts:21`. `U19` missed it because that sweep counted section- and step-level buttons only and never reached the page-level Spotlight. Under the 2026-08-23 precedence reversal (`O14`) it had become a rule violation; it no longer is.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | closed                                                     |
| `U25` | closed   | **Closed 2026-08-31 by `#112` (`b801032`), and the count this row carried was wrong twice before it landed.** Six sites on five `Transaction` pages named a field their type does not have: `noticeOfViolation` (`editorNote` and section "What to do") and `payFee` ("What to do") named **Steps**; `appealNotice` (step "Request a Director's Hearing") and `payCitation` (step "Pay by the date on the citation") named a **Transaction link**; `tenantNoticeSteps` ("Know your rights during this process") named an **Information section**. The first version of this row said five, having searched `karl` notes but not `editorNote`. The second said **seven**, and that was an over-count: its detector matched `Transaction link` as a substring, so it flagged `appealNotice`'s `editorNote` for the ordinary English phrase "a Meeting page this Transaction **links to**". Six is the measured figure. **Causation, unchanged and still hybrid:** `a743fb1` (2026-08-21) imported `payFee` and `noticeOfViolation` already carrying the phrase, six days after the 2026-08-15 sweep recorded as "resolved by rewriting the notes" — and `a743fb1` is the only commit containing `what_to_do -> Section`, so this repo never held a pre-sweep state to regress from; those sites were never clean here. `#97` (2026-08-30) then added the two new ones. A fix that never propagated, plus new pages copying a stale template. Verified after the fix: **0 residual sites**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | closed                                                     |
| `U26` | open     | **`Location` is in use and was never expanded past panel names.** `ehOffice` (5 sections) made `Location` a live type via `#97`, but the 2026-08-15 capture deliberately took it at panel-name depth only — `title`, `primary_agency`, `description`, `alert`, `image`, `body`, `intro`, `services`, `about_location`, `partner_agencies`, `at_this_location`, `people`, `related_locations`, `contact` — on the stated reasoning that nothing depended on it. Something does now. Unknown: every block internal, every chooser 's contents, and the required markers (the doc measured those only on `Step by step` and `Event`, so `Location`'s `primary_agency` requirement is **inferred, not read off the form**). `Location` has its own `services` field like `Agency`, and `U17` established that Agency's identically-labelled `resources` is a different block from everyone else's — so its `services` must not be assumed to match Agency's either. Its Scope row is marked **E2** for this reason.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | E1 — a live-admin panel expansion of `Location`            |
| `U27` | open     | **`card-inheritance`'s `INHERITS` regex reads a field described by DENIAL as a field that is present.** `src/lib/legacy-core/card-inheritance.js:32` is `/services subsection \| resources subsection \| page.{0,3} chooser/i`, and it runs over `karl` note **prose**. A note saying a type "has no per-step **page chooser**" therefore matches, and `classifySection` flips the section `unknown` → `inherits` on the strength of a negation. Found while fixing `U25` (`#112`): three sections flipped that way, inert because all three are card-less and both call sites read the result only inside `for (… of section.cards ?? [])`, and reworded to "page link" so `classifySection` output is byte-identical to `main`. The 448-test suite did not catch it. **The class is the finding, not the instance:** notes get reworded as ordinary content work, and any note that describes a Karl field by denying it can flip a classifier that matches prose. Note the same class bit the `U25` count above from the other direction — a substring match reading "Transaction **links to**" as the field name. **Whoever picks this up is deciding a data-model question, not a regex.** Measured 2026-08-31: `card-inheritance.js` imports nothing — `classifySection(section)` reads `section.karl`, the prose note, and nothing else, so no structural route exists to it today. The route is two steps and only the second is blocked. **(1)** `KARL_PANELS` already carries **105 structured `source:` matchers** mapping sections to panels, so answering _which panel is this_ structurally is available now. **(2)** Deciding _inherits vs authored_ needs the block internals, and those are **110 prose `blockTypesDoc:` strings** — lifting them into structured data is the Karl data-model refactor `#112` deliberately declined when it chose an explicit five-name list over a derived vocabulary. That restraint was right for a lint rule and is the real cost of this row. Do not read "structurally" as a small change. | Step 1 available now; step 2 is a Karl data-model decision |

**Why `U25` needed a purpose-built check rather than a vocabulary match** (measured in `#112`, landed 2026-08-31 as `src/lib/karl/crossTypeFields.test.ts`): `Transaction link` is **not a panel name**. It appears only as prose inside a `blockTypesDoc` string — `src/lib/legacy-core/karl-blocks.js:1984`, "Step block: step_type\* (number | and | or), title\*, optional, cost, time, step_description, Transaction link". Any check deriving its vocabulary from `KARL_PANELS` therefore cannot see it, however the threshold is tuned: `#112` measured the full-vocabulary version at 311 hits and a tightened version at 9, of which 8 were legitimate — and that tightened version missed **every** real defect. The landed check instead carries an explicit five-name list of fields owned by exactly one type, and matches each with `(?<![\w-])name(?![\w-])` lookarounds. **Those lookarounds are load-bearing, not decoration:** a plain substring match reads the ordinary English "this Transaction **links to**" as the field `Transaction link`, which is exactly the false positive that put the count at seven in this row's second version. Lifting the step-block fields into structured data would make the check general, but that is a Karl data-model change in service of a lint rule and was deliberately not done.
