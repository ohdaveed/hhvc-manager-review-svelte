...
| `U25` | open     | **Seven sites on five `Transaction` pages name a field their type does not have.** `noticeOfViolation` (`editorNote` **and** section "What to do") and `payFee` ("What to do") name the **Steps** field; `appealNotice` (`editorNote` **and** step "Request a Director's Hearing") and `payCitation` (step "Pay by the date on the citation") name a **Transaction link**; `tenantNoticeSteps` ("Know your rights during this process") names an **Information section**. `Steps` and `Transaction link` belong to `Step by step`, `information_section` to `Information`; `Transaction` has `what_to_do` → `Section` and nothing else. **Corrected 2026-08-31 — the causal claim in the first version of this row was wrong, and so was its count.** It said "closed once, then regressed via `#97`". Measured: `git log -S'Transaction link' -- src/lib/data/` returns **two** commits. `a743fb1` (2026-08-21) imported `payFee` and `noticeOfViolation` **already carrying the phrase — six days after the 2026-08-15 sweep this document records as "resolved by rewriting the notes"**, so for those two the sweep's result never reached this repo and they were never clean here. `#97` (2026-08-30) then added `appealNotice` and `payCitation` carrying it fresh. **Both mechanisms are real and they are different**: one is a fix that never propagated, the other is new pages copying a stale template. Either alone defeats a one-off sweep, which is why the remedy is unchanged. The count was five because the first pass searched only `karl` notes and not `editorNote`. | A validating check, not another sweep                    |
...
**Why `U25` needs a purpose-built check rather than a vocabulary match** (measured in `#112`, 2026-08-31): `Transaction link` is **not a panel name**. It appears only as prose inside a `blockTypesDoc` string — `src/lib/legacy-core/karl-blocks.js:1984`, "Step block: step_type\* (number | and | or), title\*, optional, cost, time, step_description, Transaction link". Any check that derives its vocabulary from `KARL_PANELS` therefore cannot see it, however the threshold is tuned: `#112` measured the full-vocabulary version at 311 hits and a tightened version at 9, of which 8 were legitimate — and that tightened version missed **every** real defect above. The check has to match the foreign field names as literals, not derive them.
