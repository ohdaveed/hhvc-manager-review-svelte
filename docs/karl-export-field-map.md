...
| `U25` | open     | **Five `karl` notes on `Transaction` pages name a field their type does not have — and this regressed on its own after being closed once.** `noticeOfViolation` and `payFee` ("What to do") both map to "the Transaction editor's repeatable **Steps** field"; `tenantNoticeSteps` ("Know your rights during this process") maps to an "**Information section**"; `appealNotice` ("Request a Director's Hearing") and `payCitation` ("Pay by the date on the citation") both name a **Transaction link**. None of those three fields exists on `Transaction`: `Steps` and `Transaction link` are `Step by step`, `information_section` is `Information`. Transaction has `what_to_do` → `Section` (`section_title` + `section_specifics`) and nothing else. **This is the same defect the 2026-08-15 sweep closed** — see "Step by step — the type 27 `karl` notes were describing", which records it as "Resolved by rewriting the notes". That was true when written. **Two of the five arrived with `#97`'s new pages**, so the mechanism is new pages inheriting a stale template, and it will regress again on the next batch however carefully these five are fixed. A second sweep therefore does not fix it; the fix is a check that fails when a `karl` note names a field its page type does not have. Measured 2026-08-30 at `16b04ce`. | A validating check, not another sweep                    |
