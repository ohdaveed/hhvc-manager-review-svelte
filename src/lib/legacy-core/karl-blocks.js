 ...
 */
const KARL_FLAGS = {
	// Cited by REGISTER ID, never by field-map line number. The line numbers
	// these comments used to carry ("field map line 871") had already gone stale
	// before anyone noticed — 871 pointed at the O2 row, not O14 — and the
	// 2026-08-23 precedence edits moved the rest. Ids do not drift.

	// Every Karl Callout — Transaction `what_to_do`, Transaction
	// `section_specifics`, Information `information_section` — is a single
	// Draftail field with no title (U2). A mockup `callout.title` therefore has
	// no home at all.
	calloutHasNoTitle: true,
	// All five `cost` radio variants end at the same "Cost description" rich text
	// field, capped at 120 characters (Transaction section, `cost` internals).
	costDescriptionMaxChars: 120,
	// "Bullets render inside the Text block's rich text, not as a separate
	// block" (Transaction section, practical shape).
	bulletsFoldIntoText: true,

	// REVISED 2026-08-23 with the precedence reversal (O14). The Help Center is
	// now the source of truth for how a page should be built, so 25 characters is
	// THE RULE. The live field's maxlength="255" is a gap in the form, not
	// permission: a long label saves rather than failing, and that is exactly why
	// the rule has to be carried in the tooling instead of trusted to the CMS.
	// One label in the corpus is still over — `src/lib/data/health-code-article-11.ts:21`
	// at 27 characters (U24), which now reports as a violation rather than a note.
	buttonLabelMaxChars: 25,
	// Retained so the form's real ceiling stays documented and greppable; it is
	// the permissiveness being guarded against, not a limit to author up to.
	buttonLabelFormAcceptsChars: 255,

	// Caps the FORMS do not enforce, and under the reversed precedence they are
	// rules rather than guidance: a page exceeding one can be built and saved,
	// and should not be (Spotlight table, U16).
	spotlightsAllowed: { Agency: 2, Campaign: 2, Report: 1, Topic: 1 },
	// Same shape, same reason — the Help Center caps Things to know at two
	// items; `things_to_know` has no schema maximum. Corpus is compliant:
	// 15 pages carry it, 14 with exactly 2 and one with 1.
	thingsToKnowMaxItems: 2
};
 ...
 * `https://api.sf.gov/admin/pages/add/sf/<model>/2/` (field map, Navigation
 * paths);
